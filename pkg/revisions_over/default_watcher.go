...
func NewDefaultWatcher(client kube.Client, revision string) DefaultWatcher {
	...
	p.webhooks.AddEventHandler(controllers.FilteredObjectHandler(func(o controllers.Object) {
		p.queue.AddObject(o)
	}, isDefaultTagWebhook))
...
func (p *defaultWatcher) AddHandler(handler DefaultHandler) { // 没有调用的时候
	p.mu.Lock()
	defer p.mu.Unlock()
