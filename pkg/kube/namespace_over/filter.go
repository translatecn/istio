...
func (d *discoveryNamespacesFilter) FilterAddHandler(f func(added, removed sets.String)) {
	d.lock.Lock()
	defer d.lock.Unlock()
...
func NewDiscoveryNamespacesFilter(namespaces kclient.Client[*corev1.Namespace], mesh mesh.Watcher, stop <-chan struct{}, id int) kubetypes.DynamicObjectFilter {
	// convert LabelSelectors to Selectors
	f := &discoveryNamespacesFilter{
		namespaces:            namespaces,
		discoveryNamespaces:   sets.New[string](),
		handlers:              []func(added, removed sets.String){},
		discoveryNamespacesId: id,
	}
...
func (d *discoveryNamespacesFilter) notifyHandlers(added sets.Set[string], removed sets.String) {
	...
	for _, h := range handlers {
		h(added, removed) // filteredObjectHandler
	}
	fmt.Println("over")
}
