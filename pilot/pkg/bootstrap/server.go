...
import (
	...
	"istio.io/istio/pkg/kube/namespace_over"
...
type Server struct {
	...
	secretController *multicluster.SecretController
...
func (s *Server) initSDSServer() {
	...
	if !features.EnableXDSIdentityCheck {
		...
		creds := kubecredentials.NewMulticluster(s.clusterID, s.secretController)
		creds.AddSecretHandler(func(name string, namespace string) { //  type!=helm.sh/release.v1    tpye!=kubernetes.io/service-account-token
...
// addStartFuncAndWait adds a function that should terminate before the serve shuts down
...
func (s *Server) addStartFuncAndWait(name string, fn server.Component) {
	// func (s *Server) addStartFuncAndWait(name string, fn server.Component) {	s.server.RunComponentAsyncAndWait(name, fn)
}
...
func (s *Server) initControllers(args *PilotArgs) error {
	...
	s.initSDSServer() // 监听 secret

	if features.EnableNodeUntaintControllers {
		s.initNodeUntaintController(args) // ToDo
	}

	if features.EnableIPAutoallocate {
		s.initIPAutoallocateController(args) // ToDo
	}
...
func getClusterID(args *PilotArgs) cluster.ID {
	clusterID := args.RegistryOptions.KubeOptions.ClusterID // Kubernetes
	if clusterID == "" {
...
func (s *Server) initServers(args *PilotArgs) {
	...
	multiplexGRPC := false // 多路复用
	if args.ServerOptions.GRPCAddr != "" {
		s.grpcAddress = args.ServerOptions.GRPCAddr // :15010
	} else {
	...
	h2s := &http2.Server{
		MaxConcurrentStreams: uint32(features.MaxConcurrentStreams), // 100000
	}
	...
	s.httpServer = &http.Server{
		Addr:        args.ServerOptions.HTTPAddr, // :8080
		Handler:     s.httpMux,
	...
	if args.ServerOptions.MonitoringAddr == "" { // :15014
...
func (s *Server) serveHTTP() error {
	...
	httpListener, err := net.Listen("tcp", s.httpServer.Addr) // :8080
	if err != nil {
		return err
	}
	go func() {
...
func (s *Server) initMulticluster(args *PilotArgs) {
	...
	s.secretController = multicluster.NewSecretController(
		s.kubeClient,
		args.Namespace,
		s.clusterID,
	...
	s.XDSServer.ListRemoteClusters = s.secretController.ListRemoteClusters
	s.addStartFunc("multicluster controller", func(stop <-chan struct{}) error {
		return s.secretController.Run(stop)
	})
}
...
func (s *Server) initIstiodCerts(args *PilotArgs, host string) error {
	...
	if hasCustomCertArgsOrWellKnown, tlsCertPath, tlsKeyPath, caCertPath := hasCustomTLSCerts(args.ServerOptions.TLSOptions); hasCustomCertArgsOrWellKnown {
		...
		log.Infof("initializing Istiod DNS certificates using K8S RA:%s  host: %s, custom host: %s", features.PilotCertProvider, host, features.IstiodServiceCustomHost)
...
func (s *Server) initRegistryEventHandlers() {
	...
	if s.configController != nil {
		...
		for _, schema := range schemas {
			...
			s.configController.RegisterEventHandler(schema.GroupVersionKind(), configHandler) // 这里监听了Gateway
		}
...
func (s *Server) initSecureDiscoveryService(args *PilotArgs, trustDomain string) error {
	if args.ServerOptions.SecureGRPCAddr == "" { // :15012
...
func NewServer(args *PilotArgs, initFuncs ...func(*Server)) (*Server, error) {
	e := model.NewEnvironment()                                    // xds 相关的缓存
	e.DomainSuffix = args.RegistryOptions.KubeOptions.DomainSuffix // cluster.local

	e.ServiceDiscovery = aggregate.NewController(aggregate.Options{
		MeshHolder: e,
	})
	...
	if s.kubeClient != nil {
		...
		s.kubeClient = kubelib.SetObjectFilter(s.kubeClient,
			namespace_over.NewDiscoveryNamespacesFilter(namespaces, s.environment.Watcher, s.internalStop, 1),
		)
	}
	...
	if s.kubeClient != nil {
		authenticators = append(authenticators,
			kubeauth.NewKubeJWTAuthenticator(s.environment.Watcher, s.kubeClient.Kube(), s.clusterID, s.secretController),
		)
	}
