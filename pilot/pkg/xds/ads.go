...
var knativeEnv = env.Register("K_REVISION", "", "KNative revision, set if running in knative").Get()
...
type ConnectionServer struct {
	...
	// deltaStream 用于Delta XDS。只会设置deltaStream或stream中的一个
...
func (conn *ConnectionServer) XdsConnection() *xds.Connection {
	return &conn.Connection
}

func (conn *ConnectionServer) Proxy() *model.Proxy {
	return conn.proxy
}
...
func (conn *ConnectionServer) Initialize(node *core.Node) error {
	return conn.s.initConnection(node, conn, conn.ids)
}

func (conn *ConnectionServer) Close() {
	conn.s.closeConnection(conn)
}

func (conn *ConnectionServer) Watcher() xds.Watcher {
	return conn.proxy
}

func (conn *ConnectionServer) Process(req *discovery.DiscoveryRequest) error {
	return conn.s.processRequest(req, conn)
}

// processRequest 处理一个发现请求。这是当前从主线程调用的，主线程也处理‘push’请求和关闭-代码最终将调用‘push’代码，并且它需要更多的互斥保护。原始代码通过在同一线程中同时执行“推送”和“处理请求”来避免互斥锁。
func (s *DiscoveryServer) processRequest(req *discovery.DiscoveryRequest, con *ConnectionServer) error {
	stype := v3.GetShortType(req.TypeUrl)
	log.Debugf("ADS:%s: REQ %s resources:%d nonce:%s version:%s ", stype, con.ID(), len(req.ResourceNames), req.ResponseNonce, req.VersionInfo)
...
func (s *DiscoveryServer) closeConnection(con *ConnectionServer) {
	if con.ID() == "" {
		return
	}
	s.removeCon(con.ID())
...
func (s *DiscoveryServer) pushConnection(con *ConnectionServer, pushEv *Event) error {
...
// PushOrder 定义将推送更新的顺序。此处未列出的任何类型将按随机顺序压入此处列出的类型之后
...
// StartPush Send a signal to all connections, with a push event.
...
func (conn *ConnectionServer) Clusters() []string {
	return conn.proxy.Clusters()
}

func newConnection(peerAddr string, stream DiscoveryStream) *ConnectionServer {
...
func (s *DiscoveryServer) addCon(conID string, con *ConnectionServer) {
	s.adsClientsMutex.Lock()
	defer s.adsClientsMutex.Unlock()
...
// to the tracking map.
func (s *DiscoveryServer) initConnection(node *core.Node, con *ConnectionServer, identities []string) error {
	...
	if alias, exists := s.ClusterAliases[proxy.Metadata.ClusterID]; exists { // 程序参数
...
func (s *DiscoveryServer) initializeProxy(con *ConnectionServer) error {
...
func (s *DiscoveryServer) computeProxyState(proxy *model.Proxy, request *model.PushRequest) {
	...
	if request == nil || len(request.ConfigsUpdated) == 0 || model.HasConfigsOfKind(request.ConfigsUpdated, kind.ServiceEntry) {
...
func (s *DiscoveryServer) ProxyUpdate(clusterID cluster.ID, ip string) {
	var connection *ConnectionServer
...
func (conn *ConnectionServer) watchedResourcesByOrder() []*model.WatchedResource { // 排一下顺序
...
func (conn *ConnectionServer) Push(ev any) error {
	pushEv := ev.(*Event)
