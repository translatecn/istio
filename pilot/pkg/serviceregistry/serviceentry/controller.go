...
func (s *Controller) WorkloadInstanceHandler(wi *model.WorkloadInstance, event model.Event) {
	log.Debugf("Handle event %s for workload instance (%s/%v) in namespace %s", event, wi.Kind, wi.Endpoint.Addresses, wi.Namespace)
	...
	var instancesDeleted []*model.ServiceInstance
	configsUpdated := sets.New[model.ConfigKey]()
	fullPush := false
