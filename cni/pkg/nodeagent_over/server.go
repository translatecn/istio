...
func NewServer(ctx context.Context, ready *atomic.Value, pluginSocket string, args AmbientArgs) (*Server, error) {
	...
	cfg := &iptables.Config{
		RedirectDNS:       args.DNSCapture,        // false
		EnableIPv6:        args.EnableIPv6,        // true
		TPROXYRedirection: args.TPROXYRedirection, // false
	}
	...
	hostIptables.DeleteHostRules() // 删除  ISTIO_POSTRT nat chain
...
func (s *meshDataplane) Stop() {
	...
	s.hostIptables.DeleteHostRules() // 删除  ISTIO_POSTRT nat chain
