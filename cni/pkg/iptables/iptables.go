...
type Config struct {
	...
	// 如果为true， TPROXY将用于重定向。否则，将使用REDIRECT。
	// 当前，这被视为一个特性标志，但如果有需要，可能会提升为一个永久的特性。
...
func (cfg *IptablesConfigurator) executeDeleteCommands() error {
	...
	var iptablesVariant []dep.IptablesVersion
...
func (cfg *IptablesConfigurator) DeleteHostRules() { // 删除  ISTIO_POSTRT nat chain
...
func (cfg *IptablesConfigurator) appendHostRules(hostSNATIP, hostSNATIPV6 *netip.Addr) *builder.IptablesRuleBuilder {
	...
	iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, iptablesconstants.POSTROUTING, iptablesconstants.NAT, "-j", ChainHostPostrouting)
	...
	iptablesBuilder.AppendRuleV4(iptableslog.UndefinedCommand, ChainHostPostrouting, iptablesconstants.NAT, "-m", "owner", "--socket-exists", "-p", "tcp", "-m", "set", "--match-set", fmt.Sprintf(ipset.V4Name, ProbeIPSet), "dst", "-j", "SNAT", "--to-source", hostSNATIP.String())

	// For V6 we have to use a different set and a different SNAT IP
	if cfg.cfg.EnableIPv6 {
		iptablesBuilder.AppendRuleV6(iptableslog.UndefinedCommand, ChainHostPostrouting, iptablesconstants.NAT, "-m", "owner", "--socket-exists", "-p", "tcp", "-m", "set", "--match-set", fmt.Sprintf(ipset.V6Name, ProbeIPSet), "dst", "-j", "SNAT", "--to-source", hostSNATIPV6.String())
	}
...
func (cfg *IptablesConfigurator) CreateInpodRules(log *istiolog.Scope, hostProbeSNAT, hostProbeV6SNAT netip.Addr, ingressMode bool) error {
	...
	if err := cfg.addLoopbackRoute(); err != nil { // local default dev lo scope host
		return err
	}
...
func (cfg *IptablesConfigurator) appendInpodRules(hostProbeSNAT, hostProbeV6SNAT netip.Addr, ingressMode bool) *builder.IptablesRuleBuilder {
	...
	iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, iptablesconstants.PREROUTING, iptablesconstants.MANGLE, "-j", ChainInpodPrerouting)

	// -t mangle -A OUTPUT -p tcp -j ISTIO_OUTPUT
	iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, iptablesconstants.OUTPUT, iptablesconstants.MANGLE, "-j", ChainInpodOutput)

	// -t nat -A OUTPUT -p tcp -j ISTIO_OUTPUT
	iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, iptablesconstants.OUTPUT, iptablesconstants.NAT, "-j", ChainInpodOutput)

	if redirectDNS {
		iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, iptablesconstants.PREROUTING, iptablesconstants.RAW, "-j", ChainInpodPrerouting)
		iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, iptablesconstants.OUTPUT, iptablesconstants.RAW, "-j", ChainInpodOutput)
	}
	...
	if !cfg.cfg.TPROXYRedirection {
		...
		iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, iptablesconstants.PREROUTING, iptablesconstants.NAT, "-j", ChainInpodPrerouting)
	}
	...
	if !ingressMode {
		...
		iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, ChainInpodPrerouting, iptablesconstants.MANGLE, "-m", "mark", "--mark", inpodMark, "-j", "CONNMARK", "--set-xmark", inpodTproxyMark)
		...
		iptablesBuilder.AppendVersionedRule(hostProbeSNAT.String(), hostProbeV6SNAT.String(), iptableslog.UndefinedCommand, ChainInpodPrerouting, natOrMangleBasedOnTproxy, "-s", iptablesconstants.IPVersionSpecific, "-p", "tcp", "-m", "tcp", "-j", "ACCEPT")
	}
	...
	iptablesBuilder.AppendVersionedRule(hostProbeSNAT.String(), hostProbeV6SNAT.String(), iptableslog.UndefinedCommand, ChainInpodOutput, iptablesconstants.NAT, "-d", iptablesconstants.IPVersionSpecific, "-p", "tcp", "-m", "tcp", "-j", "ACCEPT")

	if cfg.cfg.TPROXYRedirection {
		// prevent intercept traffic from app ==> app by pod ip
		// ignore traffic to localhost ip, as this rule means to catch traffic to pod ip.
		iptablesBuilder.AppendVersionedRule("127.0.0.1/32", "::1/128", iptableslog.UndefinedCommand, ChainInpodPrerouting, iptablesconstants.MANGLE, "!", "-d", iptablesconstants.IPVersionSpecific, "-p", iptablesconstants.TCP, "-i", "lo", "-j", "ACCEPT")
		...
		iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, ChainInpodPrerouting, iptablesconstants.MANGLE, "-p", "tcp", "-m", "tcp", "--dport", fmt.Sprintf("%d", ZtunnelInboundPort), "-m", "mark", "!", "--mark", inpodMark, "-j", "TPROXY", "--on-port", fmt.Sprintf("%d", ZtunnelInboundPort), "--tproxy-mark", inpodTproxyMark)
		...
		iptablesBuilder.AppendVersionedRule("127.0.0.1/32", "::1/128", iptableslog.UndefinedCommand, ChainInpodPrerouting, iptablesconstants.MANGLE, "!", "-d", iptablesconstants.IPVersionSpecific, "-p", "tcp", "-m", "mark", "!", "--mark", inpodMark, "-j", "TPROXY", "--on-port", fmt.Sprintf("%d", ZtunnelInboundPlaintextPort), "--tproxy-mark", inpodTproxyMark)
	} else if !ingressMode {
		...
		iptablesBuilder.AppendVersionedRule("127.0.0.1/32", "::1/128", iptableslog.UndefinedCommand, ChainInpodPrerouting, iptablesconstants.NAT, "!", "-d", iptablesconstants.IPVersionSpecific, "-p", "tcp", "!", "--dport", fmt.Sprint(ZtunnelInboundPort), "-m", "mark", "!", "--mark", inpodMark, "-j", "REDIRECT", "--to-ports", fmt.Sprint(ZtunnelInboundPlaintextPort))
	}
	...
	iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, ChainInpodOutput, iptablesconstants.MANGLE, "-m", "connmark", "--mark", inpodTproxyMark, "-j", "CONNMARK", "--restore-mark", "--nfmask", fmt.Sprintf("0x%x", InpodRestoreMask), "--ctmask", fmt.Sprintf("0x%x", InpodRestoreMask))
	...
	if redirectDNS {
		...
		iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, ChainInpodOutput, iptablesconstants.NAT, "!", "-o", "lo", "-p", "udp", "-m", "mark", "!", "--mark", inpodMark, "-m", "udp", "--dport", "53", "-j", "REDIRECT", "--to-port", fmt.Sprintf("%d", DNSCapturePort))
		// Same as above for TCP
		iptablesBuilder.AppendVersionedRule("127.0.0.1/32", "::1/128", iptableslog.UndefinedCommand, ChainInpodOutput, iptablesconstants.NAT, "!", "-d", iptablesconstants.IPVersionSpecific, "-p", "tcp", "--dport", "53", "-m", "mark", "!", "--mark", inpodMark, "-j", "REDIRECT", "--to-ports", fmt.Sprintf("%d", DNSCapturePort))
		...
		iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, ChainInpodOutput, iptablesconstants.RAW, "-p", "udp", "-m", "mark", "--mark", inpodMark, "-m", "udp", "--dport", "53", "-j", "CT", "--zone", "1")
		// Upstream --> Proxy return packets
		iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, ChainInpodPrerouting, iptablesconstants.RAW, "-p", "udp", "-m", "mark", "!", "--mark", inpodMark, "-m", "udp", "--sport", "53", "-j", "CT", "--zone", "1")
	}

	// CLI: -A ISTIO_OUTPUT -p tcp -m mark --mark 0x111/0xfff -j ACCEPT
	//
	// DESC: If this is outbound and has our mark, let it go.
	iptablesBuilder.AppendRule(iptableslog.UndefinedCommand, ChainInpodOutput, iptablesconstants.NAT, "-p", "tcp", "-m", "mark", "--mark", inpodTproxyMark, "-j", "ACCEPT")

	// Do not redirect app calls to back itself via Ztunnel when using the endpoint address
	// e.g. appN => appN by lo
	iptablesBuilder.AppendVersionedRule("127.0.0.1/32", "::1/128", iptableslog.UndefinedCommand, ChainInpodOutput, iptablesconstants.NAT, "!", "-d", iptablesconstants.IPVersionSpecific, "-o", "lo", "-j", "ACCEPT")
	...
	iptablesBuilder.AppendVersionedRule("127.0.0.1/32", "::1/128", iptableslog.UndefinedCommand, ChainInpodOutput, iptablesconstants.NAT, "!", "-d", iptablesconstants.IPVersionSpecific, "-p", "tcp", "-m", "mark", "!", "--mark", inpodMark, "-j", "REDIRECT", "--to-ports", fmt.Sprintf("%d", ZtunnelOutboundPort))
	return iptablesBuilder
}
